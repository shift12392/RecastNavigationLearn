...
struct rcSweepSpan
{
	unsigned short rid;	// row id  	// 一行内的id。连续的区间，id相同
	unsigned short id;	// region id      // 当前点的地区id
	unsigned short ns;	// number samples        // 采样次数
	unsigned short nei;	// neighbour id          // 下侧邻接点的地区id。
};
							...
							const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	for (int y = borderSize; y < h-borderSize; ++y)
	{
		...
		// 类似光栅化一样，从左侧到右侧逐一扫描，让右侧的地区编号尽量与左侧和下侧的保持一致。
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				if (rcGetCon(s, 0) != RC_NOT_CONNECTED)
				{
					...
					if ((srcReg[ai] & RC_BORDER_REG) == 0    // 邻接点不是地图边缘
						...
						previd = srcReg[ai];                 // 则得到左邻接点的地区编号
				}
				
				if (!previd)  // 如果左侧没有地区，那么新建一个地区。
				{
					previd = rid++;
				...
				// -y   然后判断下方邻接点，如果用左侧的邻接点没确定下来地区编号，那么判断下方邻接点。
				if (rcGetCon(s,3) != RC_NOT_CONNECTED)
				{
					const int ax = x + rcGetDirOffsetX(3);
					const int ay = y + rcGetDirOffsetY(3);
