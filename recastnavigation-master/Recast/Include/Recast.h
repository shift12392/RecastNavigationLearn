...
{
	// 这个OpenSpan串上的第一个OpenSpan在rcCompactHeightfield::spans数组的索引。
	unsigned int index : 24;	///< Index to the first span in the column.

	// 这个OpenSpan串的OpenSpan的数量
...
{
	//区间起点的y值。y+h，可以描述一段span
	unsigned short y;			///< The lower extent of the span. (Measured from the heightfield's base.)

	//区间所属地区ID，如果为0，则不属于任何地区。
...
struct rcCompactHeightfield
{
	...
	// 紧凑高度场所有的OpenSpan。
	// OpenSpan在OpenSpan串中，由底到高存放。
	// 不可行走的SolidSpan，其上方的不存在OpenSpan。
	rcCompactSpan* spans;		///< Array of spans. [Size: #spanCount]
	// 距离场
	unsigned short* dist;		///< Array containing border distance data. [Size: #spanCount]

	// 这个OpenSpan是可通过的（即这个OpenSpan脚下的SolidSpan必须有可行走标记。）
...
struct rcContourSet
{
	...
	// 世界包围盒
	// 注：如果设置了rcCompactHeightfield::borderSize，则在rcCompactHeightfield的世界包围盒的基础上减去borderSize的大小。详见rcBuildContours函数。
	float bmin[3];  	///< The minimum bounds in world space. [(x, y, z)]
	float bmax[3];		///< The maximum bounds in world space. [(x, y, z)]
	// 单元格尺寸
	float cs;			///< The size of each cell. (On the xz-plane.)
	float ch;			///< The height of each cell. (The minimum increment along the y-axis.)
	// 单元格数量
	// 注：减去了rcCompactHeightfield::borderSize的大小。详见rcBuildContours函数。
